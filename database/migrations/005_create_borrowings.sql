-- Stores the time periods for which a borrowable listing is booked.
CREATE TABLE borrowings (
    ...
    CONSTRAINT borrowings_status_check CHECK (
        status IN ('BOOKED', 'ACTIVE', 'RETURNED', 'CANCELLED')
    )
);


-- A CHECK constraint can't look at the listings table, so this trigger
-- makes sure we only create borrowings for BORROW listings.
